...
# -------------------------------------------------------
# Create facial pose nodes and sidecars from maya char files that have facial modules
def facialPoseExport(filePath, exportPath=None, mayaPyPath=None, description='facialPoseExport', sysPathAppend=None, CPU=None, debug=1, server='localhost', ignoreVersion=False):
...
#TODO: the following won't work with multiple heads on one char
faceNode = utils.getUType('faceModule')
if faceNode:
    uNode = tasks.getExportNodes()
    if uNode:
        tasks.facialPoseExport(faceNode[0], uNode[0], exportPath={1!r}, bakeRoot=False, mayaPyPath=None, sysPathAppend=None, CPU=None, debug=1)
